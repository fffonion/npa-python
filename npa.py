...
parser = argparse.ArgumentParser(description='NPA extractor')
parser.add_argument('-x', required=True,
                    dest='input_file', metavar='file', type=str,
                    help='The file to extract')
parser.add_argument('-g',
                    dest='game_id', metavar='ID', type=str,
                    default="DRAMAticalMurder",
                    choices=key_table.keys(),
                    help='Game ID')
parser.add_argument('-d',
                    dest='codec', metavar='codec', type=str,
                    default="cp932",
                    choices=("cp936", "cp932"),
                    help='String codec, use cp936 for Chinese translated and cp932 for original')
parser.add_argument('-o',
                    dest='output', metavar='dir', type=str,
                    default="output",
                    help='Output path')
parser.add_argument('-t',
                    dest='test', action='store_true',
                    default=False,
                    help='Test only, list files and exit')
args = parser.parse_args()


GAMEID = args.game_id
OUTDIR = args.output
CODEC = args.codec # cp936 for ch, cp932 for jp (original)

f = open(args.input_file, "rb")

if GAMEID not in key_table:
    print("- Game id not found, expect one of:\n  %s" % ("\n  ".join(key_table.keys())))
    sys.exit(0)

is_lamiantou = GAMEID == "Lamento" or GAMEID == "LamentoTr"

def read_bool(fd, cnt=1):
    return struct.unpack("<%d?" % cnt, f.read(cnt))

def read_int(fd, cnt=1):
    return struct.unpack("<%dI" % cnt, f.read(4*cnt))

def read_string(fd, l, file_idx):
    s = fd.read(l)
    ret = []
    for i in range(l):
        ret.append(s[i] + crypt(i, file_idx) & 0xFF)
    return bytes(ret)

# header encryption
def crypt(curnum, curfile):
    key = 0xFC * curnum
    temp = 0

    if is_lamiantou and encrypt:
        temp = key1 + key2
    else:
        temp = key1 * key2
...
# file encryption
def crypt2(curnum, name, origsize):
    i = 0
    k1 = 0
    k2 = key1 * key2
    key = 0

    if GAMEID in ("Axanael", "Kikokugai", "SonicomiTr2",
                "Sonicomi", "LostX", "DRAMAticalMurder", "DRAMAticalMurderRC"):
        k1 = 0x20101118
    elif GAMEID == "Totono":
        k1 = 0x12345678
    else:
...
# read header
#### +4
if f.read(4) != b'NPA\x01':
    print("not a NPA file")
    sys.exit(0)
#### +3
f.read(3)

#### +34
key1, key2, compress, encrypt, total_count, folder_count, file_count, start = struct.unpack(
    "<ii??III8xI",
    f.read(34),
)
print(key1, key2, compress, encrypt, total_count, folder_count, file_count, start)
if compress:
    print("* NPA is compressed")
if encrypt:
    print("* NPA is encrypted")

fdt = []
# load file table
print("* Loading content table")
for i in range(total_count):
    #### +4
    l, = read_int(f)
    #### +len
    name = read_string(f, l, i)

    #### +1
    typ, = read_bool(f)

    #### +16
    fileid, offset, compsize, origsize = read_int(f, 4)
    fdt.append((name, typ, fileid, offset, compsize, origsize))
    try:
        name.decode(CODEC)
    except UnicodeDecodeError:
        print("- codec check error, did you select the right codec (-d)?")
        sys.exit(0)

print("* Dumping files")
for i in range(len(fdt)):
    name, typ, fileid, offset, compsize, origsize = fdt[i]
    name_path = name.decode(CODEC).replace("\\", os.sep)
    if typ:
        print("%s+ %s (DIR)" % (" "*2*name_path.count(os.sep), os.path.basename(name_path)))
        if args.test:
            continue
        p = os.path.join(OUTDIR, name_path)
        if not os.path.exists(p):
            os.makedirs(p)
    else:
        print("%s'- %s" % (" "*2*name_path.count(os.sep), os.path.basename(name_path)), end="")
        if args.test:
            print("")
            continue
        buf = list(f.read(compsize))
        if encrypt:
            key = crypt2(i, name, origsize)
            x = 0
            length = 0x1000
            ...
            for j in range(min(compsize, length)):
                if is_lamiantou:
                    buf[j] = (key_table[GAMEID][buf[j]]-key) & 0xFF
                ...
                #    buffer[x] = (unsigned char) r2;
                    ...
                    buf[j] = (key_table[GAMEID][buf[j]]-key-j) & 0xFF
        if compress:
            # currently no sample to test, should be as easy as
            # zlib.decompress with correct flags
            raise Exception("NYI")
        with open(os.path.join(OUTDIR, name_path), "wb") as ff:
            ff.write(bytes(buf))
        print(" %d" % origsize)
